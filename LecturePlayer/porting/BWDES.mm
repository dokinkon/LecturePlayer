...
bool BWDES::ReadDataFromBuffer(int index, unsigned char *pBuffer, int size, unsigned char in[8])
{
    memset(in,0,8);

    int i;
    for ( i=0 ; i<8 && index*8+i<size ; i++ )
    {
        // index 為目前要處理的區塊索引, 每個區塊的大小為 8 Bytes.
        in[i] = pBuffer[index*8+i];
    }
...
bool BWDES::WriteDataToBuffer(int index, unsigned char *pBuffer, unsigned char out[8])
{
    for ( int i=0 ; i<8 ; i++ )
    {
        // index 為目前要處理的區塊索引, 每個區塊的大小為 8 Bytes.
        pBuffer[index*8+i] = out[i];
    }
    return true;
}


bool BWDES::encode(char* InputFileName, char* OutputFileName, char* KeyString)
{
    // 初始化key
    ...
    {
        Key1[i] = Md5String[i];
        Key2[i] = Md5String[i+8];
    }
    //cout << endl << Md5String << endl;
    ...
    ifstream fin(InputFileName, ios_base::binary);
    ...
    while( fin.read((char*)buf, 8) != NULL) //fin.get(buf, 8) != NULL )
    {
...
bool BWDES::decode(char* InputFileName, char* OutputFileName, char* KeyString)
{
    // 初始化key
    unsigned char Key1[8];
    unsigned char Key2[8];
...
bool BWDES::encode(unsigned char *pSrcBuffer,int SrcSize,
                unsigned char *pDestBuffer, int DestSize, char *KeyString)
{
    ...
    if ( SrcSize%8!=0 )
    {
        while ( ReadDataFromBuffer(index, pSrcBuffer, SrcSize, buf) )
        {
            des3_encrypt( &ctx3, buf, buf );
            WriteDataToBuffer(index, pDestBuffer, buf);
            index++;
        }
        des3_encrypt( &ctx3, buf, buf );
        WriteDataToBuffer(index, pDestBuffer, buf);
...
bool BWDES::decode(unsigned char *pSrcBuffer,int SrcSize,
            unsigned char *pDestBuffer, int DestSize, char *KeyString)
{
    if ( SrcSize!=DestSize )
        return false;
